...
function CheckBox(props) {
    ...
    return (
        <li Style={props.feature === 'misc' ? 'display:none' : 'display: flex; align-items: center; justify-content: space-between; font-size: 1em'}>
            <img onClick={() => tryChange(props.feature)} Style={"height:1em; display: inline"}
                 src={keyIcons[props.feature]}/>
            <label onClick={() => tryChange(props.feature)}>{props.feature}
            </label>
            <input Style="float:right" onChange={() => tryChange(props.feature)} checked={checked} name={props.feature}
                   type="checkbox"/>
        </li>
    )
}
...
function CheckBoxes() {
    ...
    return (
        <>
            <div
                Style={'position:absolute; border-radius: 5px; transition:0.5s; padding: 0.5em; margin: 0; list-style: none; z-index: 999; bottom: 0; left: 0; background: white; border: solid .1em #ddd; overflow-y:auto; max-height: 60%; margin: 0.5em '}>
                <ul>
                    {checkBoxes}
                </ul>
            </div>
